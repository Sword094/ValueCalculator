...
def calculate_value(type_input, variant_input, exist=None, rarity=None, demand=None, c=None, price=None, variant_multi=None, island_chance=None):
    x_sym = smp.Symbol('x')
    multiplier = variant_multipliers.get(variant_input, 1)

    if type_input in ["permanent", "limited"]:
        i1 = smp.integrate(smp.sqrt((rarity * multiplier) / (exist + 2)), (x_sym, 0, exist + 2))
        i2 = smp.integrate(smp.sqrt((rarity * multiplier) / exist), (x_sym, 0, exist))
        ...
        i1 = smp.integrate(smp.sqrt(const_expr / (exist + 1)), (x_sym, safe_lower, exist + 1))
        i2 = smp.integrate(smp.sqrt(const_expr / exist), (x_sym, safe_lower, exist))
        ...
        i1 = smp.integrate((price * (1 - smp.exp(-c * x_sym))) / (c / x_sym), (x_sym, 0, demand + 2))
        i2 = smp.integrate((price * (1 - smp.exp(-c * x_sym))) / (c / x_sym), (x_sym, 0, demand))
        diff = (i1 - i2) ** 1.15
        if variant_input != "normal":
...
if pet_type in ["permanent", "limited"]:
    ...
    if st.button("Calculate Value"):
        value = calculate_value(pet_type, variant, exist=exist, rarity=rarity, demand=demand)
